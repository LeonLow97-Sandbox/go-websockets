...
func (repo *DBRepo) StartMonitoring() {
	if app.PreferenceMap["monitoring_live"] == "1" {
		...
		err := app.WsClient.Trigger("public", "app-starting", data) // push to all clients in public channel
		if err != nil {
			log.Println(err)
		}
		...
		for _, x := range servicesToMonitor {
			log.Println("HostName and ServiceName -->", x.HostName, x.Service.ServiceName)
